...
const ContetnStackProductDetail = ({ category, product, isLoading, csProductContent: initCsContent }) => {
    ...
    const [csProductContent, setCsContent] = useState(initCsContent)
    ...
    /**************** Content Stack ****************/
    const productExtras = []
    console.log(csProductContent)
    if (csProductContent && csProductContent.modular_blocks) {
        for (let module in csProductContent.modular_blocks) {
            for (let block in csProductContent.modular_blocks[module]) {
                if (block) {
                    productExtras.push(
                        <RenderComponent
                            componentName={block}
                            data={csProductContent.modular_blocks[module][block]}
                        ></RenderComponent>
                    )
                }
            }
...
ContetnStackProductDetail.getTemplateName = () => 'product-detail'

ContetnStackProductDetail.shouldGetProps = ({ previousLocation, location }) => {
...
ContetnStackProductDetail.getProps = async ({ res, params, location, api, csClient }) => {
    ...
    // get content from content stack.
    console.log(csClient);
    let csProductContent = await csClient.getProductDetail(productId)
    console.log(csProductContent);

    return { category, product, csProductContent }
}

ContetnStackProductDetail.propTypes = {
    /**
    ...
    /**
    * The cs content for this product, if available.
    */
    csProductContent: PropTypes.object
}

export default ContetnStackProductDetail
